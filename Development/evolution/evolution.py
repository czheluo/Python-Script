...
class EvolutionWorkflow(Workflow):
    def __init__(self, wsheet_object):
        ...
        options = [
            ...
            {"name": "is_gwas", "type": "string", "default": "false"},  # 是否进行全基因组关联分析
        ...
        self.species_path = ''  # 根据物种，版本等拼接好的一个路径，用于后面的基因组文件上传到
        self.chr_set = 0
    ...
    def run(self):
        tasks = [self.tree, self.pca, self.ld_analysis, self.pop_history, self.sweep_analysis, self.var_diff,
                 self.go_summary]
    ...
    def import_snp_results(self):
        ...
        # snp功能统计表
        snp_api.add_sg_snp_anno_stat(self.anno_id, self.api_base_path + "/04.snp_indel/anno_stat/snp.stat", "annotation")
        # snp功效统计表
