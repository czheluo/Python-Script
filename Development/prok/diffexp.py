...
class DiffexpAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            ...
            dict(name="count", type="infile", format="prok_rna.express_matrix"),
            dict(name="exp", type="infile", format="prok_rna.express_matrix"),
    ...
    def check_options(self):
        if self.option("method").lower() not in ["degseq", "edger", "deseq2"]:
            raise OptionError("Method is incorrect", code = "35002601")
        pvalue, fc = float(self.option("pvalue")), float(self.option("fc"))
        dispersion = float(self.option("dispersion"))
        if not (0 <= pvalue <= 1):
            raise OptionError("pvalue cutoff must be in range [0-1]", code = "35002602")
        if not (0 <= dispersion <= 1):
            raise OptionError("Dispersion argument of edgeR must be in range [0-1]", code = "35002603")
        if not fc >= 0:
            raise OptionError("fold change cutoff must be positive", code = "35002604")
        ...
        if diff_groups:
            raise OptionError("Groups: %s in Compare is not in Group info", variables = (diff_groups), code = "35002605")
...
class DiffexpTool(Tool):
    ...
    def __init__(self, config):
        ...
        self.diff_toolbox = self.config.PACKAGE_DIR + "/prok_rna/diff_toolbox.py"
    ...
    def diffexp(self):
        ...
        if command.return_code == 0:
            ...
            if command.return_code is 0:
                ...
                self.set_error("%s Failed. >>> %s", variables = (cmd_name, cmd), code = "35002606")
        else:
            self.set_error("%s Failed. >>> %s", variables = (cmd_name, cmd), code = "35002607")
