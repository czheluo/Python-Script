...
class BsaWorkflow(Workflow):
    ...
    def check_base_file(self):
        ...
        if not os.path.isfile(vcf) or not os.path.isfile(summary) or not os.path.isfile(ref):
            ...
            raise Exception("文件{}或{}或{}不存在！请检查上传的原始文件是否完整！".format(vcf, summary, ref))
        else:
        ...
        if self.option("slidingwin_type") == "distance":
            ...
            if m:
                ...
                if self.win_ < self.step_:
                    ...
                    raise Exception("滑窗策略M前数值%s需大于K前数值%s!" % (self.win_, self.step_))
                ...
                raise Exception("滑窗策略%s必须是'数值M-数值K'的形式!" % self.option("slidingwin_value"))
        else:
            # self.slide_type = "num"
            ...
            if m:
                ...
                if self.win_ < self.step_:
                    ...
                    raise Exception("滑窗策略中划线前数值%s需大于中划线后数值%s!" % (self.win_, self.step_))
                self.win_ = str(self.win_)
                self.step_ = str(self.step_)
            else:
                self.set_error("滑窗策略%s必须是'正整数-正整数'的形式!", variables=(self.option("slidingwin_value")), code="11500105")
                raise Exception("滑窗策略%s必须是'正整数-正整数'的形式!" % self.option("slidingwin_value"))
